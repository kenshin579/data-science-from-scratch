...
def make_chart_simple_bar_chart():
    ...
    xs = [i + 0.1 for i, _ in enumerate(movies)] # [0.1, 1.1, 2.1, 3.1, 4.1]
    # xs = [i for i, _ in enumerate(movies)] # [0, 1, 2, 3, 4]
    print(xs)
    ...
    # plt.xticks([i + 0.5 for i, _ in enumerate(movies)], movies) # todo: <-- 0.5를 추가하면 안될 것 같음
...
def make_chart_histogram():
    ...
    print(histogram) # Counter({80: 4, 70: 3, 90: 2, 0: 2, 100: 1, 60: 1})
...
def make_chart_misleading_y_axis(mislead=True):
    ...
    plt.ticklabel_format(useOffset=False) # todo? 왜 필요한지 잘 모르겠음
...
def make_chart_scatterplot_axes(equal_axes=False):
    ...
    if equal_axes:
        ...
        plt.axis("equal") # x, y의 scale이 같게 만듬
    else:
